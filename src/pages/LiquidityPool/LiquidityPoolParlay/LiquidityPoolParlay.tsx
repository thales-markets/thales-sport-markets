...
const LiquidityPoolParlay: React.FC = () => {
    ...
    const liquidityPoolDataQuery = useParlayLiquidityPoolDataQuery(networkId, {
        enabled: isAppReady,
    });
    ...
    const userLiquidityPoolDataQuery = useParlayLiquidityPoolUserDataQuery(walletAddress, networkId, {
        enabled: isAppReady && isWalletConnected,
    });
    ...
    useEffect(() => {
        const { signer, sUSDContract, parlayAMMLiquidityPoolContract } = networkConnector;
        if (signer && sUSDContract && parlayAMMLiquidityPoolContract) {
    ...
    const handleDeposit = async () => {
        ...
        if (signer && parlayAMMLiquidityPoolContract) {
            ...
            try {
                ...
                const tx = await parlayAMMLiquidityPoolContractWithSigner.deposit(parsedAmount, {
                    gasLimit: getMaxGasLimitForNetwork(networkId),
                });
                const txResult = await tx.wait();
        ...
        if (signer && parlayAMMLiquidityPoolContract) {
            ...
            try {
                ...
                const tx = withdrawAll
                    ? await parlayAMMLiquidityPoolContractWithSigner.withdrawalRequest({
                          gasLimit: getMaxGasLimitForNetwork(networkId),
                      })
                    : await parlayAMMLiquidityPoolContractWithSigner.partialWithdrawalRequest(parsedPercentage, {
                          gasLimit: getMaxGasLimitForNetwork(networkId),
                      });
                const txResult = await tx.wait();
    ...
    return (
        <Wrapper>
                ...
                {!liquidityPoolData ? (
                    ...
                    <>
                        <MainContentContainer>
                            ...
                            <Return liquidityPoolType={'parlay'} />
                        </MainContentContainer>
                        <MainContentContainer>
                            {liquidityPoolData && (
                                <PnL
            ...
            {liquidityPoolData && <Transactions currentRound={liquidityPoolData.round} liquidityPoolType={'parlay'} />}
            {openApprovalModal && (
                <ApprovalModal
...
export default LiquidityPoolParlay;
