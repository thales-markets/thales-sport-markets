...
export const RankNumber = styled.div<{ isFirst: boolean }>`
    width: 28px;
    height: 28px;
    border-radius: 50%;
    background: ${(props) => (props.isFirst ? '#FFBE00' : '#3C498A')};
    display: flex;
    align-items: center;
    justify-content: center;
    font-family: 'Inter', sans-serif;
    font-size: 10px;
    font-weight: 600;
    color: ${(props) => (props.isFirst ? '#111325' : 'white')};
`;
...
export const FAQItem = styled.div<{ expanded: boolean }>`
    width: 1017px;
    height: ${(props) => (props.expanded ? '220px' : '56px')};
    background: #1b2141;
    border-radius: 8px;
    margin-bottom: 6px;
    overflow: hidden;
...
export const FAQToggleIcon = styled.div<{ expanded: boolean }>`
    width: 20px;
    height: 11px;
    position: relative;
    ...
    &::before {
        ...
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%) rotate(${(props) => (props.expanded ? '-45deg' : '135deg')});
