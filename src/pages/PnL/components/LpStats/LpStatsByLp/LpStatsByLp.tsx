        ...
        <Wrapper>
            <SectionWrapper>
                ...
                {lpStats.map((stats, index) => {
                    ...
                    return index === lpStats.length - 1 ? (
                        <Section key={`total-${statsName}-${index}-${networkId}-lp-by-lp`}>
                            <Label>
                        ...
                        <Section key={`${statsName}-${index}-${networkId}-lp-by-lp`}>
                            <Label>
            ...
            <SectionWrapper>
                ...
                {lpStats.map((stats, index) => {
                    const statsName = stats.name === 'CBBTC' ? 'cbBTC' : stats.name === 'WBTC' ? 'wBTC' : stats.name;
                    return index === lpStats.length - 1 ? (
                        <Section key={`total-${statsName}-${index}-${networkId}-fees-by-lp`}>
                            <Label>
                                <CurrencyIcon className="icon icon--yield" />
                                {statsName}
                            </Label>
                        ...
                        <Section key={`${statsName}-${index}-${networkId}-fees-by-lp`}>
                            <Label>
                                <CurrencyIcon className={COLLATERAL_ICONS_CLASS_NAMES[statsName as Coins]} />{' '}
                                {statsName}
                            </Label>
            ...
            <SectionWrapper>
                ...
                {lpStats.map((stats, index) => {
                    const statsName = stats.name === 'CBBTC' ? 'cbBTC' : stats.name === 'WBTC' ? 'wBTC' : stats.name;
                    return index === lpStats.length - 1 ? (
                        <Section key={`total-${statsName}-${index}-${networkId}-user-by-lp`}>
                            <Label>
                                <CurrencyIcon className="icon icon--yield" />
                                {statsName}
                            </Label>
                        ...
                        <Section key={`${statsName}-${index}-${networkId}-user-by-lp`}>
                            <Label>
                                <CurrencyIcon className={COLLATERAL_ICONS_CLASS_NAMES[statsName as Coins]} />{' '}
                                {statsName}
                            </Label>
