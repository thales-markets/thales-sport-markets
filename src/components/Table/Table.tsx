...
type CSSPropertiesWithMedia = CSSProperties & { mediaMaxWidth: string };
                ...
                <TableRowHead style={tableRowHeadStyles} {...headerGroup.getHeaderGroupProps()} key={headerGroupIndex}>
                    {headerGroup.headers.map((column: any, headerIndex: any) => (
                        <TableCellHead
                            ...
                            cssProperties={column.headStyle}
                            key={headerIndex}
                            style={
                                column.sortable
                            ...
                            id={column.id}
                        >
                            <HeaderTitle cssProperties={column.headTitleStyle}>{column.render('Header')}</HeaderTitle>
                            {column.sortable && (
                                <SortIconContainer>
                                    {column.isSorted ? (
                                        column.isSortedDesc ? (
...
const TableCellHead = styled(TableCell)<{ cssProperties?: CSSPropertiesWithMedia }>`
    font-weight: 600;
    font-size: 15px;
    ...
    @media (max-width: ${(props) => (props.cssProperties ? props.cssProperties.mediaMaxWidth : '600px')}) {
        ${(props) => (props.cssProperties ? { ...props.cssProperties } : '')}
    }
    @media (max-width: 512px) {
        font-size: 10px;
    }
...
const HeaderTitle = styled.span<{ cssProperties?: CSSPropertiesWithMedia }>`
